...
import { collection, addDoc, serverTimestamp, query, where, orderBy, limit, getDocs, doc, getDoc, onSnapshot } from 'firebase/firestore';
import { 
...
export default function TimeAttendance() {
  ...
  const [companyConfig, setCompanyConfig] = useState({
    location: null, radius: 350, greeting: { onTime: 'สวัสดีครับ', late: 'สายแล้วนะ' }
  });
  ...
  // ✅ 1. ระบบ GPS (ฉบับแก้ไข: เพิ่ม Quick Fix)
  const startGPS = (useHighAccuracy = true) => {
      if (!navigator.geolocation) {
          setLocationStatus('error');
          setGpsErrorMsg('Browser not support');
          return;
      }
      ...
      // ✨ Quick Fix: ขอพิกัดทันที 1 ครั้ง (ไม่ต้องรอ Watch)
      navigator.geolocation.getCurrentPosition(
        (position) => {
             ...
             setRetroForm(prev => ({ ...prev, location: `GPS: ${latitude.toFixed(5)}, ${longitude.toFixed(5)}` }));

             if (companyConfig.location) {
                 const dist = getDistanceFromLatLonInMeters(latitude, longitude, companyConfig.location.lat, companyConfig.location.lng);
                 setDistance(dist);
                 if (dist <= companyConfig.radius) {
        ...
        (error) => {
            console.warn("Quick GPS failed, waiting for watchPosition...");
        },
        { enableHighAccuracy: false, timeout: 5000, maximumAge: 0 } // เอาแบบหยาบๆ มาก่อนเพื่อความเร็ว
      );

      // เริ่มโหมดติดตามต่อเนื่อง (Watch)
      watchIdRef.current = navigator.geolocation.watchPosition(
          (position) => {
              ...
              if (companyConfig.location) {
                  const dist = getDistanceFromLatLonInMeters(userLat, userLng, companyConfig.location.lat, companyConfig.location.lng);
                  setDistance(dist);
              ...
              setCurrentLocation({ 
                  lat: userLat, 
                  lng: userLng, 
                  address: `Lat: ${userLat.toFixed(5)}, Lng: ${userLng.toFixed(5)} (±${Math.round(accuracy)}m)` 
              });
              setRetroForm(prev => ({ ...prev, location: `GPS: ${userLat.toFixed(5)}, ${userLng.toFixed(5)}` }));
          },
          (error) => {
              console.warn("GPS Watch Error:", error.code);
              
              if (error.code === 3 && useHighAccuracy) {
                  console.log("High accuracy timeout, trying low accuracy...");
                  startGPS(false);
              } else {
                  // ถ้ายังไม่มี Location เก่าเลย ให้ขึ้น Error (แต่ถ้า Quick Fix ทำงานไปแล้ว ก็ปล่อยผ่าน)
                  if (!currentLocation) {
                      setLocationStatus('error');
                      switch(error.code) {
  ...
  useEffect(() => {
    ...
    const fetchConfigAndStart = async () => {
        try {
            ...
            const greetingDoc = await getDoc(doc(db, "companies", currentUser.companyId, "settings", "greeting"));
            
            let config = { location: { lat: 13.7563, lng: 100.5018 }, radius: 350, greeting: { onTime: 'บันทึกเวลาสำเร็จ!', late: 'มาสายนะเรา' } };
            ...
            if (greetingDoc.exists()) {
                 ...
                 config.greeting = { onTime: gData.onTimeMessage || config.greeting.onTime, late: gData.lateMessage || config.greeting.late };
            }
            setCompanyConfig(config);

            const deductionRef = doc(db, "companies", currentUser.companyId, "settings", "deduction");
            const deductionSnap = await getDoc(deductionRef);
            if (deductionSnap.exists()) {
                setDeductionConfig(prev => ({ ...prev, ...deductionSnap.data() }));
            }
  ...
  useEffect(() => {
    ...
    const qAtt = query(collection(db, "attendance"), where("userId", "==", currentUser.uid), where("createdAt", ">=", startOfMonthDate), orderBy("createdAt", "desc"));
    ...
    const qSch = query(collection(db, "schedules"), where("userId", "==", currentUser.uid), where("date", ">=", startOfMonthStr), where("date", "<=", endOfMonthStr));
      ...
      const syncOfflineData = async () => {
          ...
          if (navigator.onLine) {
              ...
              for (const item of queue) {
                  try {
                      const docData = { ...item, createdAt: serverTimestamp() };
                      delete docData.localTimestamp; 
              ...
              if (newQueue.length === 0) {
                  ...
                  dialog.showAlert("อัปโหลดข้อมูล Offline เรียบร้อยแล้ว", "Sync Completed", "success");
                  const today = new Date(); today.setHours(0,0,0,0);
                  const q = query(collection(db, "attendance"), where("userId", "==", currentUser.uid), where("createdAt", ">=", today), orderBy("createdAt", "desc"), limit(1));
                  const snapshot = await getDocs(q);
                  if (!snapshot.empty) setTodayRecord(snapshot.docs[0].data());
              } else {
                  localStorage.setItem('offlineAttendance', JSON.stringify(newQueue));
              }
          }
      };
      syncOfflineData();
  ...
  const handleClockAction = async (type) => {
      ...
      const now = new Date();
      let isLate = false;
      if (type === 'clock-in' && todaySchedule?.startTime) {
           const [sh, sm] = todaySchedule.startTime.split(':').map(Number);
           const scheduleTime = new Date();
           scheduleTime.setHours(sh, sm + deductionConfig.gracePeriod, 0, 0); 
      ...
      const attendanceData = {
          ...
          type: type === 'clock-in' ? 'normal' : 'clock-out',
          actionType: type, 
          status: type === 'clock-out' ? 'completed' : (isLate ? 'late' : 'on-time'),
      ...
      if (!navigator.onLine) {
          try {
              ...
              const mockRecord = { ...attendanceData, createdAt: { toDate: () => now } };
              setTodayRecord(mockRecord);
          } catch (err) {
              dialog.showAlert("ไม่สามารถบันทึกข้อมูลลงเครื่องได้: " + err.message, "Error", "error");
          }
          setClocking(false);
          return;
      }
      ...
      try {
        ...
        let title = (type === 'clock-in') ? (isLate ? companyConfig.greeting.late : companyConfig.greeting.onTime) : 'เลิกงานแล้ว';
        setGreetingMessage({ title, text: '', isLate, type });
        setShowGreetingPopup(true);
        clearTimeout(popupTimeoutRef.current);
        popupTimeoutRef.current = setTimeout(() => setShowGreetingPopup(false), 5000);
      } catch (error) { 
          console.error(error); 
          dialog.showAlert("บันทึกเวลาไม่สำเร็จ: " + error.message, "Error", "error"); 
      }
      setClocking(false);
  };

  useEffect(() => { fetchTodayRecord(); }, [currentUser, clocking]);
  async function fetchTodayRecord() {
      if (!currentUser) return;
      const today = new Date(); today.setHours(0,0,0,0);
      const q = query(collection(db, "attendance"), where("userId", "==", currentUser.uid), where("createdAt", ">=", today), orderBy("createdAt", "desc"), limit(1));
      const snapshot = await getDocs(q);
      if (!snapshot.empty) setTodayRecord(snapshot.docs[0].data()); else setTodayRecord(null);
  }
  ...
  const handleRetroSubmit = async () => {
    ...
    try {
        await addDoc(collection(db, "requests"), {
           companyId: currentUser.companyId, userId: currentUser.uid, userName: currentUser.name,
           type: 'retro', status: 'pending', data: { ...retroForm }, reason: retroForm.reason, createdAt: serverTimestamp()
        });
        await dialog.showAlert("ส่งคำขอแก้เวลาย้อนหลังเรียบร้อย", "สำเร็จ", "success");
        setIsRetroModalOpen(false); setRetroForm({ date: '', timeIn: '', timeOut: '', reason: '', location: currentLocation?.address || '' });
    } catch (error) { 
