...
import { db } from '../../services/firebase'; // เพิ่ม import db เพื่อความชัวร์
import { doc, setDoc, serverTimestamp } from 'firebase/firestore'; // เพิ่ม import firestore functions
...
export default function Settings() {
  ...
  const { settings, loading: dataLoading, updateSettings } = useCompanySettings(currentUser?.companyId);
  ...
  const handleSave = async () => {
    ...
    try {
         await setDoc(doc(db, "companies", currentUser.companyId), {
            ...
            settings: {
              location: storeConfig.location, // ✅ สำคัญ!
              radius: Number(storeConfig.radius), // ✅ สำคัญ!
              gpsEnabled: storeConfig.gpsEnabled // ✅ สำคัญ!
            },
         ...
         // ส่วน Greeting
         await setDoc(doc(db, "companies", currentUser.companyId, "settings", "greeting"), {
            onTimeMessage: storeConfig.onTimeMessage,
            lateMessage: storeConfig.lateMessage,
            updatedAt: serverTimestamp()
         });
         
         // ส่วน Deduction (เผื่อไว้ถ้าอยากบันทึกรวมกันที่ปุ่มเดียว)
         await setDoc(doc(db, "companies", currentUser.companyId, "settings", "deduction"), {
            gracePeriod: Number(storeConfig.gracePeriod),
            deductionPerMinute: Number(storeConfig.deductionPerMinute),
            maxDeduction: Number(storeConfig.maxDeduction),
            updatedAt: serverTimestamp()
         }, { merge: true });
  ...
  const getCurrentLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async (pos) => { // ✅ async
        setStoreConfig(prev => ({ ...prev, location: { lat: pos.coords.latitude, lng: pos.coords.longitude } }));
        // ✅ Dialog Success
        await dialog.showAlert(`พิกัด: ${pos.coords.latitude.toFixed(4)}, ${pos.coords.longitude.toFixed(4)}`, "อัปเดตพิกัดแล้ว", "success");
      }, async (err) => { // ✅ async
         // ✅ Dialog Error
         await dialog.showAlert("GPS Error: " + err.message, "ผิดพลาด", "error");
      });
    } else {
        dialog.showAlert("Browser ไม่รองรับ GPS", "Error", "error");
    }
  };
          ...
          <div className="animate-fade-in-up space-y-6">
            ...
            <div className="modern-card p-5">
                <div className="flex justify-between items-start mb-4">
                    ...
                    <button onClick={getCurrentLocation} className="text-[10px] bg-blue-100 text-blue-600 px-2 py-1 rounded-full font-bold hover:bg-blue-200 transition">📍 ดึงพิกัดปัจจุบัน</button>
                </div>
