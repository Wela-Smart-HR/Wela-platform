...
export default function Payslip() {
  ...
  const formatMoney = (amount) => {
    return amount ? Number(amount).toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00';
  };
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where("monthId", "==", monthKey) 
    );
    const unsubscribe = onSnapshot(q, (snapshot) => {
      if (!snapshot.empty) {
        setPayslipData({ id: snapshot.docs[0].id, ...snapshot.docs[0].data() });
      } else {
        setPayslipData(null);
      }
      setLoading(false);
