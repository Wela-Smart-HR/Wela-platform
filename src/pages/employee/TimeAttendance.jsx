...
// import { db } from '../../shared/lib/firebase'; // Unused in this view
...
// ✅ Lazy Load Components (แก้ปัญหาเว็บโหลดช้า)
...
import {
    MapPin, Fingerprint, CheckCircle, X, Timer,
    WarningCircle, ClockCounterClockwise,
...
// ✅ SUB-COMPONENT 1: นาฬิกา (แยกออกมาเพื่อป้องกัน Parent Re-render)
...
// ✅ SUB-COMPONENT 2: Hold Button Logic
const HoldButton = ({ onAction, disabled, isClockIn, locationStatus }) => {
...
export default function TimeAttendance() {
    ...
    const {
        ...
        loading, // ✅ เพิ่ม Loading state
    } = useMyAttendance(currentUser?.uid, currentUser?.companyId, currentMonth);

    // ✅ Safety: ป้องกัน Crash กรณี config โหลดไม่ทัน
    const safeConfig = companyConfig || {};
    const deductionConfig = safeConfig.deduction || { gracePeriod: 5, deductionPerMinute: 10, maxDeduction: 300, employmentType: 'monthly' };

    // ===================================================
    // ⚡️ Performance Optimization (Memoization)
    // ===================================================
    const { dailyBreakdown } = useSalaryCalculator(
        attendanceLogs || [], // ✅ Safety: ป้องกัน undefined
        schedules || [],      // ✅ Safety: ป้องกัน undefined
        deductionConfig,
        currentMonth,
    ...
    const handleRetroSubmit = async () => {
        ...
        if (result.success) {
            ...
            setRetroForm({ date: '', timeIn: '', timeOut: '', reason: '', location: hookLocation?.address || '' });
        } else {
            dialog.showAlert(result.message, "Error", "error");
        }
    };
    ...
    const displayDate = new Date(); // ใช้แสดง Header
