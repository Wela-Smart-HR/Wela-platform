...
import { peopleRepo } from '@/features/people/people.repo';
...
export default function Profile() {
    ...
    const handleUpdateProfile = async () => {
        ...
        setLoading(true);
        try {
            await peopleRepo.updateEmployee(currentUser.uid, {
                    ...
                    <div className="bg-white w-full sm:max-w-sm rounded-t-[32px] sm:rounded-[32px] p-6 relative z-10 animate-slide-up shadow-2xl">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="password" autoComplete="off" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold focus:border-blue-500 outline-none" value={passwordForm.currentPassword} onChange={e => setPasswordForm({ ...passwordForm, currentPassword: e.target.value })} placeholder="ระบุรหัสปัจจุบันเพื่อยืนยัน" />
                            </div>
                            <div className="border-t border-slate-100 my-2"></div>
                            <div>
                                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">รหัสผ่านใหม่ (New Password)</label>
                                <input type="password" autoComplete="new-password" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold focus:border-emerald-500 outline-none" value={passwordForm.newPassword} onChange={e => setPasswordForm({ ...passwordForm, newPassword: e.target.value })} placeholder="********" />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">ยืนยันรหัสผ่านใหม่ (Confirm)</label>
                                <input type="password" autoComplete="new-password" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold focus:border-emerald-500 outline-none" value={passwordForm.confirmPassword} onChange={e => setPasswordForm({ ...passwordForm, confirmPassword: e.target.value })} placeholder="********" />
                            </div>
